...
#setwd("/Users/lilei/Dropbox (Morrell Lab)/Landrace_Environmental_Assocation/Analyses/cM_Mb/out/")
setwd("~/Dropbox/Projects/Wild_Introgression/Analyses/recombination/")
#cmmb <- read.table("9k_cM_100kb.txt", header=T)
cmmb <- read.table("bopa_9k_cM_100kb.txt", header=T)
...
df_new_intervals <- data.frame(
    ...
    Smoothed_cMMb=as.numeric(unlist(new_intervals["cMMb",]))
    )
