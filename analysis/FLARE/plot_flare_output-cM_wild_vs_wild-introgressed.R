...
#--------------------
# cM position
cM_pos <- as.data.frame(fread(cM_fp)) %>%
  mutate(V2=paste0("chr", V2))
# Rename columns
colnames(cM_pos) <- c("SNP", "CHROM", "cM")
...
#---------------------
prep_flare_output_cM <- function(flare_out_vcf_fp, cM_position) {
  ...
  # Add cM positions
  df_anc_cM <- left_join(df_anc, cM_position, by=c('id'='SNP', 'chr'='CHROM'))
  return(df_anc_cM)
}

# Read in FLARE output files and reformat for plotting
df_wild_introgressed <- prep_flare_output_cM(fp_wild_introgressed, cM_pos)
df_wild_rand1 <- prep_flare_output_cM(fp_wild_rand1, cM_pos)
df_wild_rand2 <- prep_flare_output_cM(fp_wild_rand2, cM_pos)
df_wild_rand3 <- prep_flare_output_cM(fp_wild_rand3, cM_pos)
...
# Not all SNPs with physical positions have cM positions, remove SNPs without cM pos
prep_introg_regions_cM <- function(df_anc) {
  df_introgressed_regions <- df_anc %>%
    filter(!is.na(cM)) %>%
    group_by(rleid(ancestry), ancestry, chr, sample, gt, an1, an2) %>%
    arrange(`rleid(ancestry)`, cM) %>%
    summarise(start = gdata::first(cM), end=gdata::last(cM), .groups='drop') %>%
    dplyr::select(chr, start, end, sample, gt, an1, an2, ancestry) %>%
    as.data.frame()
  # Add size of introgressed intervals
  df_introgressed_regions$cM_length <- df_introgressed_regions$end - df_introgressed_regions$start
  return(df_introgressed_regions)
}

regions_wild_introgressed <- prep_introg_regions_cM(df_wild_introgressed)
regions_wild_rand1 <- prep_introg_regions_cM(df_wild_rand1)
regions_wild_rand2 <- prep_introg_regions_cM(df_wild_rand2)
regions_wild_rand3 <- prep_introg_regions_cM(df_wild_rand3)
...
# Save file
write_delim(df_comb, file="ibs_cM_wild_vs_wild-introgressed.txt", delim="\t", quote="none")
...
ggplot(df_comb, aes(x=Dataset, y=cM_length)) +
...
# Save plot
ggsave("ibs_cM_wild_vs_wild-introgressed_jitterplot-anc_hom_ref.jpg", width=12, height=10, units="in", dpi=300)

ggplot(df_comb, aes(x=chr, y=cM_length, fill=Dataset)) +
...
# Save plot
ggsave("ibs_cM_wild_vs_wild-introgressed_by_chr_boxplot-anc_hom_ref.jpg", width=14, height=12, units="in", dpi=300)
