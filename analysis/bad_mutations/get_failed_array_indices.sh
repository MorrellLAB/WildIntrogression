...
#   bad mutations predict FAILED array indices
...
#------------------------------
# Generate a list of failed bad_mutations predict array indices to re-run
...
sacct -j ${JOB_ID} --format="JobID%15,JobName,State,ExitCode" | \
    grep -v 'JobID\|-----\|extern\|batch\|COMPLETED' | grep "FAILED" | \
