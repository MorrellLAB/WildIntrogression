...
python ${GENOMICS_GENERAL}/popgenWindows.py -w ${WIN_SIZE} -m ${MIN_SITES_PER_WIN} -g ${GENO_FILE} --outFile ${OUT_DIR}/${OUT_PREFIX}.win${WIN_SIZE}.minsites${MIN_SITES_PER_WIN}.csv.gz -f phased -T 5 -p wild -p wild_introgressed -p domesticated --popsFile ${POPS_FILE}
